-- The exploration hub is a second production UI, not a reduced-data preview.
-- Match the main site's expression policy: every ACTIVE expression is available.
-- Edge Story source content remains excluded by dialogue.story_only.
...
relation_rows as (
  ...
  join public.expression e on e.id = rb.target_id and e.status = 'ACTIVE'
...
association_rows as (
  ...
  join public.expression e on e.id = ga.target_expression_id and e.status = 'ACTIVE'
  join public.expression_meaning em on em.expression_id = e.id and em.is_primary
...
category_rows as (
  ...
  cross join limits l
  cross join lateral (
    select ec.expression_id, row_number() over (order by ec.id) member_order
    from public.expression_category ec
    where ec.category_id = c.id and ec.expression_id <> a.id
    limit l.n
  ) ranked
  join public.expression member on member.id = ranked.expression_id and member.status = 'ACTIVE'
  join public.expression_meaning mem on mem.expression_id = member.id and mem.is_primary
),
dialogue_rows as (
...
comment on function public.get_expression_hub(bigint, integer) is
  'ACTIVE expression projection for the second production exploration UI; edge Story sources stay excluded.';
