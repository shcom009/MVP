-- Use a concise first-line phrase as the group label only when the complete
-- phrase occurs verbatim in an active member pronunciation or meaning.

create or replace function public.source_memo_exact_phrase_label(
...
begin
  ...
  if char_length(v_candidate) not between 4 and 20 then
    return null;
  end if;
...
revoke all on function public.source_memo_exact_phrase_label(text, text)
